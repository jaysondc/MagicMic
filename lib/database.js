...
export const addSong = (title, artist, options = {}) => {
    const now = Date.now();
    const { album_cover_url = null, audio_sample_url = null, duration_ms = null } = options;

    // Check if options is inadvertently passed as part of legacy calls, though unlikely
    // Ideally we should just rely on this new signature.

    const result = db.runSync(
        `INSERT INTO songs (title, artist, album_cover_url, audio_sample_url, duration_ms, status, my_rating, sing_count, created_at, updated_at) 
     VALUES (?, ?, ?, ?, ?, 'to_try', 0, 0, ?, ?)`,
        [title, artist, album_cover_url, audio_sample_url, duration_ms, now, now]
    );
    return result.lastInsertRowId;
};
