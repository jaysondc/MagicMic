...
export const runMigrations = () => {
    try {
        // Check if audio_sample_url column exists, if not add it
    ...
    db.withTransactionSync(() => {
        ...
        data.forEach(song => {
            const songId = db.runSync(
                `INSERT INTO songs (title, artist, album_cover_url, audio_sample_url, status, my_rating, sing_count, created_at, updated_at) 
         VALUES (?, ?, ?, ?, 'to_try', 0, 0, ?, ?)`,
                [song.title, song.artist, song.album_cover_url || null, song.audio_sample_url || null, now, now]
            ).lastInsertRowId;
