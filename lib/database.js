...
export const unlinkTagFromSong = (songId, tagId) => {
    db.runSync('DELETE FROM song_tags WHERE song_id = ? AND tag_id = ?', [songId, tagId]);
};

// Get all song IDs for a specific tag
export const getSongIdsForTag = (tagId) => {
    const results = db.getAllSync('SELECT song_id FROM song_tags WHERE tag_id = ?', [tagId]);
    ...
    db.withTransactionSync(() => {
        ...
        data.forEach(song => {
         ...
         VALUES (?, ?, ?, ?, ?, ?, 'to_try', 0, 0, ?, ?)`,
            ...
            // Auto-tagging logic removed in favor of high-quality seed data
